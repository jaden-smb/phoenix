...
// Field types: u8/i8/u16/i16/u32/i32/f32. Records are packed at natural C alignment so the
// generated POD struct matches the blob by construction (a static_assert in the header guards it).
...
inline bool build_bin(BundleWriter& w, const std::string& in, const std::string& name = "",
    ...
    if (!header_out.empty()) {
        ...
        for (const BinField& f : fs) std::fprintf(h, "    %-9s %s;\n", bin_ctype(f.type), f.name.c_str());
